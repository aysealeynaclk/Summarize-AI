...
async function handleSummarize() {
  ...
  try {
    ...
    error.value = err.response?.data?.detail || 'Özetleme sırasında bir hata oluştu.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="max-w-3xl mx-auto px-4 py-10">
    <h1 class="text-2xl font-bold text-slate-800 mb-1">Metnini Özetle</h1>
    <p class="text-slate-500 mb-6">Bir paragraf yapıştır, saniyeler içinde kısa bir özet al.</p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
      ...
      <div v-if="summary" class="bg-indigo-50 border border-indigo-100 rounded-lg px-4 py-3">
        <p class="text-xs font-medium text-indigo-500 mb-1">ÖZET</p>
        <p class="text-slate-800 whitespace-pre-line">{{ summary }}</p>
      </div>
    </div>

    <div v-if="recentSummaries.length" class="mt-8">
      <h2 class="text-sm font-semibold text-slate-600 mb-3">Son 3 Özetin</h2>
      <ul class="space-y-3">
        <li
